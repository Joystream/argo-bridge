...
function isWeb3Injected(injectedWindow: InjectedWindow): boolean {
  return (
    Object.values(injectedWindow.injectedWeb3 || {}).filter(
      ({ connect, enable }) => !!(connect || enable)
    ).length !== 0
  )
}
...
class InjectedWallet implements BaseWallet {
  ...
  async getAccounts(): Promise<Account[]> {
    ...
    let walletAccounts = injectedAccounts?.map((account) =>
      toWalletAccount(account)
    )
  ...
  async connect() {
    try {
      ...
      console.error(`Error initializing ${this.metadata.title}: ${e.message}`)
      throw e
    }
  }
...
export class InjectedWalletProvider implements BaseWalletProvider {
    ...
    if (injectedWindow.injectedWeb3) {
      Object.keys(injectedWindow.injectedWeb3).forEach((extensionId) => {
        if (!this.config.disallowed?.includes(extensionId)) {
          const foundExtension = this.config.supported?.find(
            ({ id }) => id === extensionId
          )
          if (foundExtension) {
            knownExtensions.push({
              ...injectedWindow.injectedWeb3[extensionId],
              metadata: foundExtension,
            })
          } else {
            otherExtensions.push({
              ...injectedWindow.injectedWeb3[extensionId],
              metadata: { id: extensionId, title: extensionId },
            })
          }
        }
      })
  ...
  async getWallets(): Promise<BaseWallet[]> {
    ...
    injectedWallets = extensions.map(
      (ext) => new InjectedWallet(ext, this.appName)
    )
