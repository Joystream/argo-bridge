...
export const POLKADOT_CHAIN_ID = 'polkadot:6b5e488e0fa8f9821110d5c13f4c468a'
...
class WalletConnectWallet implements BaseWallet {
  ...
  async connect() {
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      const unsubscribeModal = this.walletConnectModal.subscribeModal(
        (state: ModalState) => {
          if (state.open === false) {
            unsubscribeModal()
            resolve()
...
export class WalletConnectProvider implements BaseWalletProvider {
  ...
  getWallets(): Promise<BaseWallet[]> {
    return new Promise((resolve) =>
      resolve([new WalletConnectWallet(this.config, this.appName)])
    )
  }
}
